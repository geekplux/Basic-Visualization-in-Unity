...
{
    public static Mesh CreateMesh(Vector2[] points, float width, string orien = "xz")
    {
        Vector2[] verts = CreateLineVerticies(points, width);
        int[] triangles = CreateTriangles(points.Length);

        Mesh mesh = new Mesh();
        mesh.vertices = ConvertVector2ToVector3(verts, orien);
    ...
    public static Vector3[] ConvertVector2ToVector3(Vector2[] points, string orien)
    {
        switch (orien)
        {
            case "xy":
                return points.Select(it => new Vector3(it.x, it.y, 0)).ToArray();
            case "yz":
                return points.Select(it => new Vector3(0, it.x, it.y)).ToArray();
            default:
